-- 1 player
-- 1 ship_type
-- item_types, bonus_types, weapon_types (см. описание таблиц)
-- 1 engine, bonuses of all types, weapons of all types ==> items of all types
-- 1 location
...
alter table inventories_items
        ...
        add constraint fk_inventories_items_player foreign key (player_inventory_id) references players (player_inventory_id);

alter table items
        add constraint fk_item_item_type foreign key (item_type_id) references item_types (item_type_id);

alter table players
        add constraint fk_player_inventory foreign key (player_inventory_id) references inventories_items (player_inventory_id),
